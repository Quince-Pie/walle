...
static bool initialize_device(struct walle_vk_renderer* renderer, VkSurfaceKHR surface)
{
    ...
    const char*                        requested             = getenv("WALLE_VK_DEVICE");
    ...
    if (!selected) {
        fprintf(stderr,
                ...
                requested ? " matching WALLE_VK_DEVICE=" : "",
                requested ? requested : "");
        return false;
    }
    ...
    fprintf(stderr,
            "[Vulkan] %s, API %u.%u.%u, SPIR-V 1.6, dynamic rendering, synchronization2.\n",
