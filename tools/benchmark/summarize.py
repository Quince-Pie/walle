...
METRICS=('gpu_total_ns','gpu_frame_ns','gpu_capture_ns','gpu_draw_ns','gpu_tail_ns',
...
def summarize(path):
    ...
    assert all(r['status']=='PASS' and r['validation_errors']==0 and r['owned_bytes_after_output_destroy']==0 for r in ends)
    runs=[]
    warm_all=[];first=[]
    for run in range(1,6):
        ...
        assert all(not r['built_backdrop'] and r['gpu_capture_ns']==0 for r in warm)
    ...
    return dict(case=case,device=identities[0],device_sha256=device_hash,
