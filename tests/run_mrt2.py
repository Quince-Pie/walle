#!/usr/bin/env python3
"""Verify native MRT2 state, transient ownership and pre-recording failure policy without a GPU."""
import argparse
import hashlib
import json
import os
...
def main() -> int:
    ...
    build = (args.build_dir or root / "build/tests/mrt2" / args.profile).resolve()
    build.mkdir(parents=True, exist_ok=True)
    inputs = [root / "vulkan_renderer.c", root / "vulkan_renderer.h",
              tests / "mrt2_contracts.c", tests / "transition_check.h", Path(__file__).resolve(),
    ...
    try:
        ...
        flags = ["-std=c23", "-DWALLE_RETAIN_DISCARD_ATTACHMENT=1", "-ffp-contract=off", "-fno-fast-math", "-Wall", "-Wextra", "-Wpedantic",
        ...
        wrappers = ("vkCreateImage", "vkGetImageMemoryRequirements", "vkAllocateMemory", "vkBindImageMemory",
                    ...
                    "vkCreateShaderModule", "vkDestroyShaderModule", "vkCreateGraphicsPipelines", "vkCreateComputePipelines")
        binary = build / "mrt2"
        command([*cc, *flags, str(tests / "mrt2_contracts.c"),
