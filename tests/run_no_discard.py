#!/usr/bin/env python3
"""Verify DISCARD-only elision, live tint/SDF resources and unchanged failure policy without a GPU."""
import argparse
import hashlib
import json
import os
...
def main() -> int:
    ...
    build = (args.build_dir or root / "build/tests/no_discard" / args.profile).resolve()
    build.mkdir(parents=True, exist_ok=True)
    inputs = [root / "vulkan_renderer.c", root / "vulkan_renderer.h",
              tests / "no_discard_contracts.c", tests / "transition_check.h", Path(__file__).resolve(),
    ...
    try:
        ...
        flags = ["-std=c23", "-DWALLE_RETAIN_DISCARD_ATTACHMENT=0", "-ffp-contract=off", "-fno-fast-math", "-Wall", "-Wextra", "-Wpedantic",
        ...
        wrappers = ("vkCreateImage", "vkGetImageMemoryRequirements", "vkAllocateMemory", "vkBindImageMemory",
                    ...
                    "vkGetPhysicalDeviceImageFormatProperties", "vkCmdPipelineBarrier2", "vkCmdBeginRendering",
                    "vkCmdSetRenderingAttachmentLocations", "vkCmdSetRenderingInputAttachmentIndices",
                    "vkCreateShaderModule", "vkDestroyShaderModule", "vkCreateGraphicsPipelines", "vkCreateComputePipelines", "vkCreateDescriptorPool")
        binary = build / "no_discard"
        command([*cc, *flags, str(tests / "no_discard_contracts.c"),
