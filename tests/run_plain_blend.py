#!/usr/bin/env python3
"""Verify hardware-blend capability fallback, pipeline state, specialization and failure cleanup without a GPU."""
import argparse
import hashlib
import json
import os
...
def main() -> int:
    ...
    build = (args.build_dir or root / "build/tests/plain-blend" / args.profile).resolve()
    build.mkdir(parents=True, exist_ok=True)
    inputs = [root / "vulkan_renderer.c", root / "vulkan_renderer.h",
              tests / "plain_blend_contracts.c", tests / "transition_check.h", Path(__file__).resolve(),
    ...
    try:
        ...
        wrappers = ("calloc", "free", "vkGetPhysicalDeviceFormatProperties2", "vkCreateShaderModule", "vkDestroyShaderModule", "vkCreateGraphicsPipelines", "vkCreateComputePipelines", "vkDestroyPipeline", "vkDeviceWaitIdle", "vkDestroyDevice")
        binary = build / "plain-blend"
        command([*cc, *flags, str(tests / "plain_blend_contracts.c"),
