#!/usr/bin/env python3
"""Verify ramp eligibility, hardware sampler state, and absence of a shared scalar buffer without a GPU."""
import argparse
import hashlib
import json
import os
...
def main() -> int:
    ...
    build = (args.build_dir or root / "build/tests/native-ramp" / args.profile).resolve()
    build.mkdir(parents=True, exist_ok=True)
    inputs = [root / "vulkan_renderer.c", root / "vulkan_renderer.h",
              tests / "native_ramp_contracts.c", tests / "transition_check.h", Path(__file__).resolve(),
              *sorted((root / "material").glob("*.h")),
              *(root / "shaders" / name for name in ("native_tint_ramp.rgba16f", "native_tint_gradient.bin"))]
    ...
    try:
        ...
        flags = ["-std=c23", "-fno-builtin-memcmp", "-ffp-contract=off", "-fno-fast-math", "-Wall", "-Wextra", "-Wpedantic",
        ...
        wrappers = ('memcmp', 'vkCreateSampler', 'vkDestroySampler', 'vkCreateCommandPool', 'vkDestroyCommandPool', 'vkAllocateCommandBuffers', 'vkCreateFence', 'vkDestroyFence', 'vkCreateBuffer', 'vkDeviceWaitIdle', 'vkDestroyDevice', 'vkCreateDescriptorSetLayout', 'vkDestroyDescriptorSetLayout', 'vkCreateDescriptorPool', 'vkDestroyDescriptorPool', 'vkResetDescriptorPool', 'vkAllocateDescriptorSets', 'vkUpdateDescriptorSets', 'vkCmdPipelineBarrier2', 'vkCmdCopyBufferToImage')
        binary = build / "native-ramp"
        command([*cc, *flags, str(tests / "native_ramp_contracts.c"),
