...
private func render(
    ...
    for draw in plan.draws {
        ...
        guard words[base] == UInt32(draw.x),
              ...
              words[base + 2] == 0,
              words[base + 3] == UInt32(draw.targetIndex),
              words[base + 4] == UInt32(draw.recordIndex),
              words[base + 5] == UInt32(draw.targetRecordIndex),
              words[base + 6] == UInt32(draw.sampleRecordIndex),
              words[base + 7] == UInt32(draw.patternIndex),
              words[base + 8] == UInt32(draw.tileX),
              words[base + 9] == UInt32(draw.tileY),
              words[base + 10] == UInt32(draw.sampleOrdinal),
              words[base + 11] == UInt32(draw.targetIndex)
        else {
            throw ProbeError.command("record \(draw.recordIndex) identity differs")
        }
    }
    ...
    return (
        ...
        [
            ...
            "compile": [
                ...
                "drawScissor": "one authenticated interior pixel per record",
            ],
        ]
    )
}
