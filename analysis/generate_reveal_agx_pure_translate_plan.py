...
OUTPUT_DEFAULT: Final = (
    ROOT / "build" / "analysis-agx-basis" / "pure-translate-plan-v1"
)
...
def geometries():
    out = []
    # s31-o6 SHAPE, whole triangle translated in 1-subpixel steps in x;
    # placed so the centroid stays on-screen (v2 x = x0+1049 well inside).
    for k in range(256):
        x0 = -530.0 - k / 256.0
        out.append(((x0 + 1049.0, 614.5), (x0 + 1049.0, 1663.5), (x0, 1663.5)))
    # same, translated in y (crossing bottom? keep on-screen y): translate
    # the o2 shape vertically with all vertices moving together
    for k in range(256):
        y0 = -530.0 - k / 256.0
        out.append(((1696.0, y0), (1696.0, y0 + 1184.0), (512.0, y0 + 1184.0)))
    return out
