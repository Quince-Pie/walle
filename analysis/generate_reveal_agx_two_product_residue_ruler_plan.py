...
"""Generate the controlled-residue M1 AGX two-product ruler plan."""
...
def generate(output_directory: Path) -> JsonObject:
    ...
    ruler.VARIABLE_HIGH_BITS = 0x3F800000
    ruler.generate(output_directory)
    manifest_path = output_directory / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["schema"] = "walle-reveal-agx-two-product-residue-ruler-plan-manifest-v1"
    manifest["generator"] = {
    ...
    manifest["sharedGenerator"] = {
        "path": ruler.Path(ruler.__file__).relative_to(ROOT).as_posix(),
        "bytes": ruler.Path(ruler.__file__).stat().st_size,
        "sha256": _sha256(ruler.Path(ruler.__file__)),
    }
