...
OUTPUT_DEFAULT: Final = (
    ROOT / "build" / "analysis-agx-basis" / "single-clip-ruler-v5b-plan-v1"
)
...
def generate(output_directory: Path) -> dict:
    ...
    values = []
    for js in range(0, 64):
        e = model.bits_f32(_perturb(model.f32_bits(0.75), js))
        values.append(e)
    ...
    plan_path.write_text(json.dumps(plan, indent=2, sort_keys=True) + "\n",
                         encoding="utf-8")
    manifest = {
        "schema": "walle-reveal-agx-single-clip-ruler-v5b-plan-manifest-v1",
        "generator": {
