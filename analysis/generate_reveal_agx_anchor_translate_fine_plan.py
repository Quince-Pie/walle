...
OUTPUT_DEFAULT: Final = (
    ROOT / "build" / "analysis-agx-basis" / "anchor-translate-fine-plan-v1"
)
...
def geometries():
    out = []
    # o2-shape: anchor y swept in 1-subpixel steps over 1.5 px around -700
    for k in range(384):
        y0 = -700.0 - k / 256.0
        out.append(((1696.0, y0), (1696.0, 614.5), (512.0, 614.5)))
    # o6-shape x-family: anchor x swept in 1-subpixel steps around -560
    for k in range(384):
        x0 = -560.0 - k / 256.0
