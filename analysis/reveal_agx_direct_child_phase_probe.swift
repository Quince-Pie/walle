...
private let expectedCanonicalDirectInputSHA256 =
    "c698b11432704c47eb9c4acf396aa03c5dd3ee78a94d133f000e7c0091677052"
private let expectedLocalOneHotDirectInputSHA256 =
    "b094620680091bf3fff0dbfe9fb9edeabfc88eb62a86e8764260190481c6010c"
...
private func flattenCatalog(
    ...
    guard draws.count == catalog.census.sampleRecordCount,
...
private func render(
    ...
    guard let device = MTLCreateSystemDefaultDevice(),
          ...
          let target = makeTarget(
              device: device,
              width: catalog.target.width,
              height: catalog.target.height
          ),
          let directVertexBuffer = directVertexData.withUnsafeBytes({ buffer in
              device.makeBuffer(
        ...
        throw ProbeError.resource("Metal device, queue, target, or direct vertex buffer")
    }
    ...
    var transform = matrix(width: catalog.target.width, height: catalog.target.height)
    encoder.setRenderPipelineState(pipeline)
    encoder.setViewport(MTLViewport(
        originX: 0,
        originY: 0,
        width: Double(catalog.target.width),
        height: Double(catalog.target.height),
        znear: 0,
        zfar: 1
    ))
    encoder.setVertexBuffer(directVertexBuffer, offset: 0, index: 0)
    ...
    return (
        ...
        [
            ...
            "compile": [
                ...
                "geometryPath": "direct canonical post-guard child",
                "pullLattice": [
...
private func run(
    ...
    let directInputData = try Data(contentsOf: directInputURL)
    let directInputSHA256 = sha256(directInputData)
    guard directInputData.count == catalog.census.sampleRecordCount * 3 * 32,
          directInputSHA256 == expectedCanonicalDirectInputSHA256
            || directInputSHA256 == expectedLocalOneHotDirectInputSHA256
    else {
        throw ProbeError.invalid("direct-child input identity")
    }
    let basisMode = directInputSHA256 == expectedCanonicalDirectInputSHA256
        ? "canonical-transport"
        : "local-onehot"
    let draws = try flattenCatalog(catalog)
    let (output, runtime) = try render(
        catalog: catalog,
        directVertexData: directInputData,
        draws: draws
    )
    ...
    var manifest: [String: Any] = [
        ...
        "directChildInput": [
            "file": directInputURL.lastPathComponent,
            "bytes": directInputData.count,
            "sha256": directInputSHA256,
            "basisMode": basisMode,
            "layout": "record-major, three 32-byte vertices",
        ],
        "capture": [
            "file": outputName,
            "bytes": output.count,
        ...
        do {
            guard CommandLine.arguments.count == 4 else {
                throw ProbeError.invalid(
                    "catalog, direct-child input, and output-directory arguments"
                )
            }
            try run(
                catalogURL: URL(fileURLWithPath: CommandLine.arguments[1]),
                directInputURL: URL(fileURLWithPath: CommandLine.arguments[2]),
                outputDirectory: URL(
