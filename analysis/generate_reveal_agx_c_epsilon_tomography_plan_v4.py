...
one binade; v4 samples column 64 where the x-part is 3/4 eps and
the net lever eps/4: with k = bl-63 every eps term is an exact
multiple of the 4v accumulator quantum (anchor 16v, x-part 12v per
unit), so the scan is strictly monotone: step 4v, span 4G, exact
grid readout.
...
OUTPUT_DEFAULT: Final = (
    ROOT / "build" / "analysis-agx-basis" / "c-epsilon-tomography-plan-v4"
)
...
def generate(output_directory: Path) -> dict:
    ...
    plan = {
        ...
        "authority": {
            ...
            "establishesCEpsilonTomographyV4": True,
        },
