...
OUTPUT_DEFAULT: Final = (
    ROOT / "build" / "analysis-agx-basis" / "apex-x-sweep-plan-v1"
)
...
def geometries():
    out = []
    # fixed crossing y-geometry (den 1328: y_in 613, y_out -715), sweep V2.x
    y_in, y_out = 613.0, -715.0
    for k in range(120):
        x2 = 672.0 + k * 4.0 + (k % 8) / 8.0   # includes fractional pixels
        out.append(((640.0, y_in), (1600.0, 913.0), (x2, y_out)))
    # second family: den 2048 (y_out -1435)
    for k in range(60):
        x2 = 672.0 + k * 9.0 + (k % 4) / 4.0
        out.append(((640.0, y_in), (1600.0, 913.0), (x2, y_in - 2048.0)))
    return out
...
def generate(output_directory: Path) -> dict:
    ...
    plan_path.write_text(json.dumps(plan, indent=2, sort_keys=True) + "\n",
                         encoding="utf-8")
    manifest = {
        "schema": "walle-reveal-agx-apex-x-sweep-plan-manifest-v1",
        "generator": {
