...
OUTPUT_DEFAULT: Final = (
    ROOT / "build" / "analysis-agx-basis" / "c-truthtable2-plan-v1"
)
VERTEX: Final = struct.Struct("<8I")
ROWS: Final = range(17, 64)
COLUMN: Final = 60

V0 = (0x44000000, 0x44000000)   # (512, 512)  anchor: didx = (ty-16)*8192
V1 = (0x45200000, 0x44000000)   # (2560, 512)
V2 = (0x45200000, 0x45900000)   # (2560, 4608): diagonal slope 2, det = 2^39
...
def probe_words():
    words = [0x3F800000]                          # 1.0 exact
    for j in range(1, 24):                        # one-hot low bit
        words.append(0x3F800000 | (1 << (23 - j)))
    for a in (14, 16, 18, 20, 22):                # bit pairs
        for b in (4, 8, 12, 15):
            if 23 - b <= 23 - a:
                continue
            words.append(0x3F800000 | (1 << (23 - a)) | (1 << (23 - b)))
    for mbits in (2, 4, 8, 16, 23):               # dense tails
        words.append(0x3F800000 | ((1 << mbits) - 1))
    words.append(0x3F7FFFFF)                      # just below 1.0
    words.append(0x3FFFFFFF)                      # 1.99...
    return words
