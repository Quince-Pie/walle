...
Per record the four varying channels carry (s, 0), (0, e), (s, e), (s, 0.5):
with one endpoint zero the hardware word reads a single interpolator product
directly, isolating the u x s and t x e product laws from each other.
...
OUTPUT_DEFAULT: Final = (
    ROOT / "build" / "analysis-agx-basis" / "single-clip-ruler-v4-plan-v1"
)
...
def generate(output_directory: Path) -> dict:
    ...
    for geometry_index, geometry in enumerate(geometries):
        ...
        for s in values:
            record = len(draws)
            pairs = [(s, 0.0), (0.0, E_VALUE), (s, E_VALUE), (s, 0.5)]
    ...
    plan_path.write_text(json.dumps(plan, indent=2, sort_keys=True) + "\n",
                         encoding="utf-8")
    manifest = {
        "schema": "walle-reveal-agx-single-clip-ruler-v4-plan-manifest-v1",
        "generator": {
