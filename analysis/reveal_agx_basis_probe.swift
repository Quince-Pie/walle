    ...
    device uint4 *results [[buffer(2)]])
{
    ...
    const float2 plusX16 = tileOrigin + float2(16.0f, 0.0f);
    const float2 minusX16 = tileOrigin - float2(16.0f, 0.0f);
    const float2 plusY16 = tileOrigin + float2(0.0f, 16.0f);
    const float2 minusY16 = tileOrigin - float2(0.0f, 16.0f);
    const float2 plusX1 = tileOrigin + float2(1.0f, 0.0f);
    const float2 plusY1 = tileOrigin + float2(0.0f, 1.0f);
    ...
    results[base + 5u] =
        as_type<uint4>(input.basis.interpolate_at_offset(tileOrigin));
    results[base + 6u] =
        as_type<uint4>(input.basis.interpolate_at_offset(plusX16));
    results[base + 7u] =
        as_type<uint4>(input.basis.interpolate_at_offset(minusX16));
    results[base + 8u] =
        as_type<uint4>(input.basis.interpolate_at_offset(plusY16));
    results[base + 9u] =
        as_type<uint4>(input.basis.interpolate_at_offset(minusY16));
    results[base + 10u] =
        as_type<uint4>(input.basis.interpolate_at_offset(plusX1));
    results[base + 11u] =
        as_type<uint4>(input.basis.interpolate_at_offset(plusY1));
    results[base + 12u] = as_type<uint4>(dfdx(center));
    results[base + 13u] = as_type<uint4>(dfdy(center));
    results[base + 14u] =
        as_type<uint4>(input.basis.interpolate_at_offset(float2(0.0f)));
    return 1.0f;
}
"""
...
private func run(catalogURL: URL, outputDirectory: URL) throws {
    ...
    let outputName = "reveal-agx-basis.raw"
    try output.write(
    ...
    var manifest: [String: Any] = [
        "schema": "walle-reveal-agx-basis-capture-v1",
        "authority": [
        ...
        "capture": [
            ...
            "ordering": "catalog recordIndex, 15 uint4 vectors",
            "vectors": [
                ...
                "ITER center basis",
                "LDCF basis at tile origin",
                "LDCF basis at tile origin + X16",
                "LDCF basis at tile origin - X16",
                "LDCF basis at tile origin + Y16",
                "LDCF basis at tile origin - Y16",
                "LDCF basis at tile origin + X1",
                "LDCF basis at tile origin + Y1",
                "DFDX center basis",
                "DFDY center basis",
                "LDCF basis at fragment center",
            ],
        ],
        "executable": [
...
@main
private struct RevealAGXBasisProbe {
    static func main() {
        do {
