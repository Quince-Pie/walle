#!/usr/bin/env python3
"""Single-clip interpolation ruler v6: divider readout via e near 1.0.

With s = 0 and e swept in single-ulp steps around exactly 1.0, the solved
clip-vertex word is round_half_up_24(t_hw * e); intersecting the boundary
constraints across the sweep reads the divider output t_hw to ~2^-29
relative precision with no multiplier-family structure in the way.

Geometry groups:
  A. the 64 v5c (y_in x y_out) pairs (pin t_hw for the v5c wobble study);
  B. coarse denominator sweep at num=1125: den = 1137+16k (64 samples);
  C. dense denominator patch at num=1125: den = 1264 + k/32 (32 samples);
  D. shared-den pairs: den = 1312+8k for k<32 reached from BOTH
     y_in=613 (num=1125) and y_in=800 (num=1312) - separates a
     reciprocal-of-den law from a joint (num,den) divider law.
"""
...
OUTPUT_DEFAULT: Final = (
    ROOT / "build" / "analysis-agx-basis" / "single-clip-ruler-v6-plan-v1"
)
...
def build_geometries():
    ...
    for y_in in Y_INSIDE:
        for y_out in Y_OUTSIDE:
            geoms.append(("A", y_in, y_out))
    for k in range(64):
        den = 1137 + 16 * k
        geoms.append(("B", 613.0, -(den - 613.0)))
    for k in range(32):
        den = 1264.0 + k / 32.0
        geoms.append(("C", 613.0, -(den - 613.0)))
    for k in range(32):
        den = 1312 + 8 * k
        geoms.append(("D", 613.0, -(den - 613.0)))
        geoms.append(("D", 800.0, -(den - 800.0)))
    return geoms
...
def generate(output_directory: Path) -> dict:
    ...
    plan_path.write_text(json.dumps(plan, indent=2, sort_keys=True) + "\n",
                         encoding="utf-8")
    manifest = {
        "schema": "walle-reveal-agx-single-clip-ruler-v6-plan-manifest-v1",
        "generator": {
