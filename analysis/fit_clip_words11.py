...
def snap(v):
    ...
    return _snap0(v)
POSQ = [False]
CANDS = [("exact/vq24rtz/posexact", t_exact_model, 24, "rtz", False)]
if True:
    pass
for name, tm, W, md, sf in CANDS:
    VMODE[0] = md; SNAPF32[0] = False; POSQ[0] = (sf == "posq")
    VQ[0] = W
    ...
    for gi, hws in sorted(geo_hw.items()):
        ...
        for ctx in range(3):
            ...
            for tri in itertools.combinations(range(n), 3):
                ...
                got = fused_AB(fx3, vals)
                if got == hw:
                    found = True; break
                if got is not None and None not in got:
                    d = sum(abs((a & 0x7fffffff) - (b & 0x7fffffff))
                            + (0 if (a >> 31) == (b >> 31) or a == 0 or b == 0 else 1000)
                            for a, b in zip(got, hw))
                    if best is None or d < best[0]:
                        best = (d, tri, got)
            ok += found
            per_ctx[ctx][0] += found; per_ctx[ctx][1] += 1
            if not found and best is not None and name.endswith("posexact"):
                d, tri, got = best
                da = (hw[0] & 0x7fffffff) - (got[0] & 0x7fffffff)
                db = (hw[1] & 0x7fffffff) - (got[1] & 0x7fffffff)
                print(f"MISS g{gi} c{ctx} tri={tri} dA={da:+d} dB={db:+d}")
    print(f"{name}: {ok}/{tot}  " +
