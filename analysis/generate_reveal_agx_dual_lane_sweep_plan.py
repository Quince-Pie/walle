...
OUTPUT_DEFAULT: Final = (
    ROOT / "build" / "analysis-agx-basis" / "dual-lane-sweep-plan-v1"
)
...
def geometries():
    ...
    out = []
    # Base right triangles at several scales; apex perturbed in subpixels.
    for base in (96.0, 355.0, 1024.0, 1421.0):
        for height in (96.0, 512.0, 1150.5):
            x0, y0 = 640.0, 640.0
            for dx256 in (-64, -17, -8, -3, -2, -1, 0, 1, 2, 3, 8, 17, 64):
                for dy256 in (-64, -9, -1, 0, 1, 9, 64):
                    apex = (x0 + base + dx256 / 256.0,
                            y0 + height + dy256 / 256.0)
                    out.append(((x0, y0), (x0 + base, y0), apex))
    # Slivers along diagonals (both-axis nonzero edges with fine ratios).
    for k in range(48):
        x0, y0 = 512.0, 512.0
        w = 700.0 + k * 3.0 + (k % 7) / 256.0
        h = 500.0 - k * 2.0 + (k % 11) / 256.0
