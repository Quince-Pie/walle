#!/usr/bin/env python3
"""Dense-tile capture for every triangle owning a corpus residual pixel.
...
OUTPUT_DEFAULT: Final = (
    ROOT / "build" / "analysis-agx-basis" / "residual-children-dense-plan-v1"
)
...
def generate(output_directory: Path) -> dict:
        ...
        for ty in range(ty0, ty1 + 1):
            for tx in range(tx0, tx1 + 1):
                ...
                experiments.append({
                    "recordIndex": record,
                    "inputOrdinal": record,
                    "variant": "residual-children-dense",
                    "split": "discovery",
                    "state": state,
                    "drawOrdinal": ordinal,
                    "anchor": an,
    ...
    plan_path.write_text(json.dumps(plan, indent=2, sort_keys=True) + "\n",
                         encoding="utf-8")
    manifest = {
        "schema": "walle-reveal-agx-residual-children-dense-plan-manifest-v1",
        "generator": {
