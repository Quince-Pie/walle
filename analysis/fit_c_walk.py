...
for key in [(40, 2, 0), (40, 2, 1), (58, 4, 0)]:
    ...
    best = None
    for seed in [(txs[0], tys[0]), (txs[-1], tys[0]), (txs[0], tys[-1]),
                 (txs[len(txs)//2], tys[0])]:
        if seed not in tiles: 
            # nearest available in that row
            cand = [t for t in tiles if t[1] == seed[1]]
            if not cand: continue
            seed = min(cand, key=lambda t: abs(t[0]-seed[0]))
        seedv = frac_of(tiles[seed])
        for Wq in (24, 25, 26, 28, None):
            sx = quantW(Af * 32, Wq); sy = quantW(Bf * 32, Wq)
            for Wa in (24, 25, 26, 28, None):
                ok = 0
                for (tx, ty), hwC in tiles.items():
                    # walk: y first from seed, then x, quantizing per step
                    v = seedv
                    step_dir = 1 if ty >= seed[1] else -1
                    for _ in range(abs(ty - seed[1])):
                        v = quantW(v + step_dir*sy, Wa)
                    step_dir = 1 if tx >= seed[0] else -1
                    for _ in range(abs(tx - seed[0])):
                        v = quantW(v + step_dir*sx, Wa)
                    if rne24_word(v) == hwC: ok += 1
                if best is None or ok > best[0]:
                    best = (ok, seed, Wq, Wa)
    print(f"{key}: best walk fit {best[0]}/{len(tiles)} seed={best[1]} Wq={best[2]} Wa={best[3]}")
