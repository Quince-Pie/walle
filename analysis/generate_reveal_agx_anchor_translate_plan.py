...
OUTPUT_DEFAULT: Final = (
    ROOT / "build" / "analysis-agx-basis" / "anchor-translate-sweep-plan-v1"
)
...
def geometries():
    out = []
    # s31-o6-shape triangle (e_x = 0 on basis0's axis-0) translated so the
    # anchor sweeps across suspected thresholds: y in -900..-500 and the
    # x-crossing family across x in -900..-400.
    for dy in range(0, 400, 8):
        y0 = -500.0 - dy          # anchor y
        out.append(((512.0, y0), (512.0, y0 + 1049.0), (1561.0, y0 + 1049.0)))
    for dx in range(0, 400, 8):
        x0 = -400.0 - dx
        out.append(((x0, 614.5), (x0, 1663.5), (x0 + 1049.0, 1663.5)))
    # o2-shape (vertical right column, e_y = 0 for basis2)
    for dy in range(0, 400, 8):
        y0 = -500.0 - dy
        out.append(((1696.0, y0), (1696.0, 614.5), (512.0, 614.5)))
    return out
...
def generate(output_directory: Path) -> dict:
    ...
    plan_path.write_text(json.dumps(plan, indent=2, sort_keys=True) + "\n",
                         encoding="utf-8")
    manifest = {
        "schema": "walle-reveal-agx-anchor-translate-sweep-plan-manifest-v1",
        "generator": {
