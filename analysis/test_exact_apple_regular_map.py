...
sigma = math.hypot(width, height) * 0.038 # Regular blur sigma
...
hw_reg_arr = np.array(hw_reg_img, dtype=np.float32)

y, x = np.ogrid[:2000, :3200]
circle_mask = (x - 1600)**2 + (y - 1000)**2 <= 240**2

blur_c = pil_arr[circle_mask]
hw_c = hw_reg_arr[circle_mask]

# Solve least-squares linear fit per channel for Regular material
a_r, b_r = np.polyfit(blur_c[:, 0], hw_c[:, 0], 1)
a_g, b_g = np.polyfit(blur_c[:, 1], hw_c[:, 1], 1)
a_b, b_b = np.polyfit(blur_c[:, 2], hw_c[:, 2], 1)

print(f"Fitted Regular linear map Red:   HW_R = {a_r:.4f} * Blur_R + {b_r:.2f}")
print(f"Fitted Regular linear map Green: HW_G = {a_g:.4f} * Blur_G + {b_g:.2f}")
print(f"Fitted Regular linear map Blue:  HW_B = {a_b:.4f} * Blur_B + {b_b:.2f}")
...
print(f"\nFitted Regular Affine Map vs Apple Native HW Regular (Circle Interior MAE): {mae_fitted:.2f}")
print(f"Fitted Mean RGB inside circle: {fit_sim[circle_mask].mean(axis=0)}")
print(f"Apple Native HW Regular Mean RGB inside circle: {hw_c.mean(axis=0)}")
