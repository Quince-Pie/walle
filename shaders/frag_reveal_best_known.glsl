...
uniform sampler2D RevealMask; // recovered public-input reveal coverage (R8)

// --- Uniforms ---
uniform float Time;              // LINEAR normalized [0,1]
uniform vec2  Resolution;        // buffer px
uniform float Variant;           // 0.0 = clear, 1.0 = regular
uniform vec2  RevealCenterPointPixels;
uniform float RevealRadiusPixels;
...
void main() {
    ...
    vec2  delta     = frag_px - RevealCenterPointPixels;
    float dist_px   = length(delta);
    float radius_px = RevealRadiusPixels;
    float sdf       = dist_px - radius_px;
    vec2  n_out     = dist_px > 0.5 ? delta / dist_px : vec2(0.0, 1.0);

    // The dedicated pass renders the recovered two-family geometry into an R8
    // mask using the current GLES approximation.  FBO texels share
    // gl_FragCoord's bottom-left origin, so texelFetch is one-to-one.
    float mask = texelFetch(RevealMask, ivec2(gl_FragCoord.xy), 0).r;
    ...
    if (mask <= 0.0 && sdf > shadow_off + shadow_pen + 8.0) {
    ...
    float sdf_sh
        = distance(frag_px - vec2(0.0, -shadow_off), RevealCenterPointPixels) - radius_px;
    ...
    vec2  ring_c   = RevealCenterPointPixels - vec2(0.0, RING_OFFSET_FRAC * radius_px);
